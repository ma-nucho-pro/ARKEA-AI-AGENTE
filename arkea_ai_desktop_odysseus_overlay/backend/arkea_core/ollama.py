...
from backend.arkea_core.security import guarded_request, validate_outbound_url
try:
    from backend.arkea_core.db import get_setting
except Exception:
    get_setting = None

SAFE_LOCAL_OLLAMA_URL = "http://127.0.0.1:11434"
DEFAULT_OLLAMA_BASE_URL = os.getenv("OLLAMA_BASE_URL", SAFE_LOCAL_OLLAMA_URL)
...
def base_url():
    if get_setting:
        try:
            configured = (get_setting('ollama_base_url', DEFAULT_OLLAMA_BASE_URL) or DEFAULT_OLLAMA_BASE_URL).rstrip("/")
            return validate_outbound_url(configured, allow_local=True)
        except Exception:
            return SAFE_LOCAL_OLLAMA_URL
    try:
        return validate_outbound_url(DEFAULT_OLLAMA_BASE_URL, allow_local=True)
    except Exception:
        return SAFE_LOCAL_OLLAMA_URL
...
def server_ok(timeout=0.45):
    try:
        r = guarded_request("GET", f"{base_url()}/api/tags", allow_local=True, timeout=timeout)
...
def pull_model(model_id: str):
    ...
    try:
        r = guarded_request("POST", f"{base_url()}/api/pull", allow_local=True, json={'model': model_id, 'stream': False}, timeout=1800)
        r.raise_for_status()
...
def warm_model(model_id: str | None = None):
    ...
    try:
        ...
        r = guarded_request("POST", f"{base_url()}/api/generate", allow_local=True, json=payload, timeout=90)
...
def delete_model(model_id: str):
    ...
    try:
        r = guarded_request("DELETE", f"{base_url()}/api/delete", allow_local=True, json={'model': model_id}, timeout=90)
        _OLLAMA_MODELS_CACHE.update({'ts': 0.0, 'value': []})
        _OLLAMA_STATUS_CACHE.update({'ts': 0.0, 'value': None})
...
def chat_local(message: str, system: str = "Eres ARKEA AI.", model: str | None = None):
    ...
    def call_model(mid, timeout_s=80, max_tokens=240):
        ...
        r = guarded_request("POST", f"{base_url()}/api/chat", allow_local=True, json=payload, timeout=timeout_s)
        r.raise_for_status()
