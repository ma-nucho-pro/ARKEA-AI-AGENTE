...
const ARKEA_API_TOKEN = document.querySelector('meta[name="arkea-api-token"]')?.content || '';
const nativeFetch = window.fetch.bind(window);
window.fetch = (input, init={}) => {
  const target = new URL(typeof input === 'string' ? input : input.url, window.location.href);
  if(ARKEA_API_TOKEN && target.origin === window.location.origin && target.pathname.startsWith('/api/')){
    const headers = new Headers(init.headers || (typeof input !== 'string' ? input.headers : undefined));
    headers.set('X-Arkea-Token', ARKEA_API_TOKEN);
    init = {...init, headers};
  }
...
let removeOmnirouteProgress = null;
...
function setPreviewHtml(html, trustedEditable=false){ showViewFrame(); currentPreviewKind=trustedEditable?'canvas':'html'; viewer.setAttribute('sandbox', trustedEditable?'allow-same-origin':'allow-scripts'); viewer.removeAttribute('src'); viewer.srcdoc = html || ''; currentPreviewUrl = ''; }
function setPreviewUrl(url,label='Vista previa'){ showViewFrame(); currentPreviewKind='url'; currentPreviewUrl=url||''; viewer.setAttribute('sandbox','allow-scripts'); viewer.removeAttribute('srcdoc'); viewer.src=url||'about:blank'; setPreviewLabel(label); }
...
function setPreviewImage(url,label='Imagen generada'){ showViewFrame(); currentPreviewKind='image'; currentPreviewUrl=url||''; viewer.setAttribute('sandbox',''); viewer.removeAttribute('src'); viewer.srcdoc = _imageViewerHtml(url,label); setPreviewLabel(label); updateViewerZoomLabel(); }
...
function updateEngineBadge(){
  const mode = (settingsCache.ai_engine_mode || 'auto').toLowerCase();
  const labels = {auto:'AUTO · OMNI → LOCAL',free:'GRATIS · OMNIROUTE',local:'LOCAL · OLLAMA',direct:'DIRECTA · APIs'};
  const badge = $('#engineBadge');
  if(badge){ badge.dataset.mode = mode; badge.textContent = labels[mode] || labels.auto; }
}
async function loadSettings(){
  try{
    ...
    applyTheme(); applyAvatarSettings(); updateEngineBadge(); await loadVoices();
...
async function saveSetting(k,v){
  ...
  applyTheme(); applyAvatarSettings(); updateEngineBadge();
}
...
function openCanvasEditor(initial=''){
  ...
  setPreviewHtml(html, true); setPreviewLabel('Canvas editable');
}
...
async function send(){
  ...
  try{
    ...
    timeout = setTimeout(()=>controller.abort(), Number(settingsCache.chat_timeout_ms || 120000));
    const data = await j('/api/arkea/chat',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({message:backendText, mode: visualMode ? 'visual_html' : 'auto', conversation_id: currentConversationId, project_id: currentProjectId, skill_id: settingsCache.active_skill_id || null}),
...
async function openSettingsTab(tab='workspace'){
  ...
  if(tab === 'universal-ai') return renderUniversalAI();
...
function loadPreview(url,label){ setPreviewUrl(url,label); closeSettings(); }
...
async function renderUniversalAI(){
  ...
  const mode = config.mode || settingsCache.ai_engine_mode || 'auto';
  const models = config.models || {};
  const running = Boolean(gateway.running || desktop.running);
  const installed = Boolean(desktop.installed || gateway.installed);
  const lastDecision = config.last_decision || 'Todavía no hay una ruta registrada.';
  $('#settingsContent').innerHTML = `
    ...
    <div class="card">
      <b>Elige cómo trabaja toda la infraestructura</b>
      <div class="engine-grid">
        <label class="engine-option"><input type="radio" name="aiEngineMode" value="auto" ${mode==='auto'?'checked':''}/><span><b>Automático</b><small>OmniRoute primero, después tus conexiones y Ollama como último recurso local.</small></span></label>
        <label class="engine-option"><input type="radio" name="aiEngineMode" value="free" ${mode==='free'?'checked':''}/><span><b>IA gratis</b><small>Solo OmniRoute. No consume tus APIs directas ni cambia silenciosamente a otro motor.</small></span></label>
        <label class="engine-option"><input type="radio" name="aiEngineMode" value="local" ${mode==='local'?'checked':''}/><span><b>IA local</b><small>Solo Ollama: privado y usable sin internet.</small></span></label>
        <label class="engine-option"><input type="radio" name="aiEngineMode" value="direct" ${mode==='direct'?'checked':''}/><span><b>Cualquier API directa</b><small>OpenAI, Anthropic/Gemini mediante gateway compatible, OpenRouter o tu endpoint.</small></span></label>
      </div>
    </div>
    <div class="card form-grid">
      ...
      <div><label>Documentos</label><input id="omniModelDocument" value="${esc(models.document || 'auto')}"/></div>
      <button data-action="save-universal-ai" class="primary">Guardar motor y modelos</button>
      <button data-action="connect-omniroute">Conectar a todas las capacidades</button>
    </div>
    <div class="card"><b>Último enrutado</b><div class="path">${esc(lastDecision)}</div><p class="muted">OmniRoute conecta muchos proveedores y aplica fallback. “Gratis” depende de las cuotas y términos de cada proveedor; no significa ilimitado.</p></div>
    <div id="omniModelList" class="model-cloud-list"></div>`;
  $$('input[name="aiEngineMode"]').forEach(input => input.onchange = async () => {
    await configureUniversalAi(input.value);
  });
}
...
async function configureUniversalAi(forcedMode=''){
  const selected = forcedMode || $('input[name="aiEngineMode"]:checked')?.value || settingsCache.ai_engine_mode || 'auto';
  const body = {
    mode:selected,
    base_url:$('#omniBaseUrl')?.value || 'http://127.0.0.1:20128',
    api_key:$('#omniApiKey')?.value || null,
    models:{
      chat:$('#omniModelChat')?.value || 'auto',
      code:$('#omniModelCode')?.value || 'auto/coding',
      artifact:$('#omniModelCode')?.value || 'auto/coding',
      vision:$('#omniModelVision')?.value || 'auto',
      document:$('#omniModelDocument')?.value || 'auto',
      file:$('#omniModelDocument')?.value || 'auto',
      web_search:'auto',
      image_generation:'auto'
    }
  };
  await j('/api/arkea/omniroute/configure',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
  settingsCache.ai_engine_mode = selected;
  updateEngineBadge();
  return body;
}
...
async function connectOmniroute(){
  await configureUniversalAi();
  const result = await j('/api/arkea/omniroute/apply',{method:'POST'});
  alert(`OmniRoute conectado: ${result.created || 0} nuevas y ${result.updated || 0} actualizadas.`);
  await renderUniversalAI();
}

async function loadOmnirouteModels(){
  const box = $('#omniModelList');
  if(box) box.innerHTML = '<span class="muted">Consultando modelos y combos reales…</span>';
  const data = await j('/api/arkea/omniroute/models');
  if(box) box.innerHTML = (data.models || []).slice(0,120).map(model => `<code>${esc(model.id || model.name || JSON.stringify(model))}</code>`).join('') || '<span class="muted">OmniRoute no devolvió modelos todavía. Configura proveedores en su panel.</span>';
}
...
async function renderApis(){
  ...
  $('#settingsContent').innerHTML = `<h3>APIs directas · cualquier proveedor compatible</h3><p class="muted">Guarda cada API por separado. Para Anthropic, Gemini y otros protocolos usa su endpoint OpenAI-compatible o conéctalos en OmniRoute.</p>
  ...
  <div class="card">
    ...
    <button data-action="fill-api-provider" data-type="vision" data-provider="openrouter_nex_vision_free">Llenar visión gratis Nex</button>
    <button data-action="fill-api-provider" data-type="vision" data-provider="openrouter_vision_step_flash">Llenar visión barata StepFun</button>
  </div>
...
async function saveElevenQuick(){
  ...
  const values = {elevenlabs_voice_name:voiceName, elevenlabs_voice_id:voiceId, use_elevenlabs_voice:$('#use_elevenlabs_voice').value};
  if(apiKey) values.elevenlabs_api_key = apiKey;
  await saveBulk(values);
  if(apiKey && voiceId){
...
function stopAllLive(){
  ...
  stopLiveBuildPreview();
  abortLiveRequests();
  try{ activeChatController?.abort(); }catch{}
...
async function renderSkills(){
  ...
  const active = d.active_skill_id || '';
  $('#settingsContent').innerHTML = `<h3>Skills globales del agente</h3><div class="card ok"><b>Aplicación real</b><p>La skill seleccionada se agrega al contexto de chat, código, documentos, visión, búsqueda, archivos e imágenes.</p><button data-action="activate-skill" data-id="">Usar sin skill específica</button></div><div class="card"><label>Crear skill con IA/reglas</label><textarea id="skillPrompt" placeholder="create skill para hacer tesis con estructura UCV..."></textarea><button data-action="create-skill">Crear skill .md</button></div><div class="card"><b>Subir skill .md</b><input type="file" id="skillFile" accept=".md,text/markdown,text/plain"/><button data-action="upload-skill">Subir e instalar Markdown</button></div>${(d.skills || []).map(s=>`<div class="card ${active===s.skill_id?'ok':''}"><b>${esc(s.name)} ${active===s.skill_id?'· ACTIVA':''}</b><p>${esc(s.description || '')}</p><div class="path">${esc(s.folder_path)}</div><div class="actions"><button data-action="activate-skill" data-id="${esc(s.skill_id)}">${active===s.skill_id?'Activa para todo':'Activar para todo'}</button><button data-action="toggle-skill" data-id="${esc(s.skill_id)}">${s.enabled?'Desactivar':'Activar'}</button></div></div>`).join('') || '<p>Sin skills.</p>'}`;
}
...
async function activateSkill(id){ await j('/api/arkea/skills/activate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({skill_id:id || ''})}); settingsCache.active_skill_id=id||''; await renderSkills(); }
async function toggleSkill(id){ await j(`/api/arkea/skills/${encodeURIComponent(id)}/toggle`,{method:'POST'}); await renderSkills(); }
async function renderMcp(){
  const d = await j('/api/arkea/mcp');
  const a = await j('/api/arkea/automation/status');
  $('#settingsContent').innerHTML = `<h3>Herramientas MCP ejecutables</h3>
    <div class="card warn"><b>Permisos y confirmación</b><p>${esc(a.message)}</p>
      <button data-action="toggle-control" data-enabled="${!a.computer_control_enabled}">${a.computer_control_enabled?'Desactivar':'Activar'} control de PC</button>
      <button data-action="toggle-browser" data-enabled="${!a.browser_agent_enabled}">${a.browser_agent_enabled?'Desactivar':'Activar'} agente navegador</button>
    </div>
    <div class="form-grid card">
      <input id="mcp_name" placeholder="filesystem-mcp"/>
      <input id="mcp_command" placeholder="npx"/>
      <input id="mcp_args" placeholder='["-y","@modelcontextprotocol/server-filesystem","C:\\ruta"]'/>
      <input id="mcp_permissions" placeholder="execute,read,write,filesystem"/>
      <small>Permisos admitidos: read, write, network, execute, browser, filesystem.</small>
      <label class="toggle"><span><b>Confirmar llamadas</b><small>Si se desmarca, el agente puede invocar las tools permitidas automáticamente.</small></span><input type="checkbox" id="mcp_confirm" checked/></label>
      <button data-action="add-mcp">Agregar MCP</button>
    </div>
    ${(d.mcp_servers || []).map(m=>`<div class="card"><b>${esc(m.name)} · ${m.enabled?'activo':'desactivado'}</b>
      <div class="path">${esc(m.command)} ${esc(m.args || '')}</div>
      <p>Permisos: ${esc(m.permissions || '[]')}</p>
      <p>${m.requires_confirmation?'Requiere confirmación; no se ofrece al bucle automático.':'Puede participar en el bucle agente→MCP dentro de sus permisos.'}</p>
      <div class="actions"><button data-action="list-mcp-tools" data-id="${m.id}">Probar y listar tools</button><button data-action="toggle-mcp" data-id="${m.id}">${m.enabled?'Desactivar':'Activar'}</button></div>
      <div id="mcp-tools-${m.id}" class="model-cloud-list"></div></div>`).join('') || '<p>Sin MCPs.</p>'}`;
}
...
async function addMcp(){ const raw = ($('#mcp_args')?.value || '').trim(); let args=[]; if(raw){ try{ args=JSON.parse(raw); if(!Array.isArray(args)) args=[String(args)]; }catch{ return alert('Los argumentos deben ser un array JSON, por ejemplo ["-y","paquete"].'); } } const allowed=['read','write','network','execute','browser','filesystem']; const permissions=($('#mcp_permissions')?.value||'').split(',').map(x=>x.trim().toLowerCase()).filter(Boolean); if(!permissions.length||permissions.some(x=>!allowed.includes(x))||!permissions.includes('execute')) return alert('Incluye execute y solo permisos válidos: read, write, network, execute, browser, filesystem.'); await j('/api/arkea/mcp/add',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:$('#mcp_name').value || 'mcp-personalizado',command:$('#mcp_command').value || 'npx',args,permissions,requires_confirmation:$('#mcp_confirm')?.checked !== false})}); openSettingsTab('mcp'); }
async function listMcpTools(id){ const box=$(`#mcp-tools-${id}`); if(box)box.innerHTML='<span>Inicializando MCP…</span>'; const d=await j(`/api/arkea/mcp/${id}/tools`); if(box)box.innerHTML=(d.tools||[]).map(t=>`<code><b>${esc(t.name)}</b><br>${esc(t.description||'')}</code>`).join('')||'<span>Servidor conectado, sin tools publicadas.</span>'; }
async function toggleMcp(id){ await j(`/api/arkea/mcp/${id}/toggle`,{method:'POST'}); await renderMcp(); }
...
async function runDiagnostics(){
  ...
  const requiredActions = ['new-chat','choose-folder','open-folder','install-omniroute','start-omniroute','connect-omniroute','save-api','test-api','install-ollama','search-ollama-models','pull-by-id','create-skill','upload-skill','activate-skill','list-mcp-tools','choose-obsidian','save-appearance'];
  const results = [];
  ...
  const endpoints = ['/api/health','/api/arkea/settings','/api/arkea/conversations','/api/arkea/apis','/api/arkea/omniroute/status','/api/arkea/skills','/api/arkea/mcp','/api/arkea/ollama/catalog'];
...
async function finishOnboarding(){
  ...
  const installFree = $('#onboardInstallFree')?.checked;
...
async function handleAction(e){
  ...
  const longActions = new Set(['install-omniroute','start-omniroute','load-omniroute-models','connect-omniroute','pull-model','pull-by-id','pull-defaults','install-required-pack','warm-selected-model','save-model-prefs','delete-selected-model','test-api','install-whisper','create-skill','upload-skill','save-api','load-eleven-voices','list-mcp-tools']);
  ...
  try{
    ...
    if(a === 'open-universal-ai') await openSettingsTab('universal-ai');
    if(a === 'install-omniroute') await installOmniroute();
    if(a === 'start-omniroute') await startOmniroute();
    if(a === 'open-omniroute') await window.arkeaDesktop?.openOmniroute?.();
    if(a === 'load-omniroute-models') await loadOmnirouteModels();
    if(a === 'save-universal-ai') { await configureUniversalAi(); await renderUniversalAI(); }
    if(a === 'connect-omniroute') await connectOmniroute();
    if(a === 'fill-api-provider'){
      $('#api_type').value=b.dataset.type || 'chat'; fillProviders();
      $('#api_provider_select').value=b.dataset.provider || 'custom';
      $('#api_provider_select').onchange();
    }
    ...
    if(a === 'activate-skill') await activateSkill(b.dataset.id || '');
    if(a === 'toggle-skill') await toggleSkill(b.dataset.id || '');
    ...
    if(a === 'list-mcp-tools') await listMcpTools(Number(b.dataset.id));
    if(a === 'toggle-mcp') await toggleMcp(Number(b.dataset.id));
...
Object.assign(window, {send, uploadSelectedFiles, createNewChat, openConversation, openPath, chooseWorkspaceAndNewChat, openSettingsTab, closeSettings, loadPreview, renderUniversalAI, installOmniroute, startOmniroute, connectOmniroute, loadOmnirouteModels, saveApiConnection, applyApiPreset, deleteApi, testApiConnection, saveElevenQuick, loadElevenVoicesFromApi, selectElevenVoice, installWhisper, installOllama, refreshOllama, installRequiredPack, saveModelPrefs, deleteSelectedModel, warmSelectedModel, stopAllLive, searchOllamaModels, pullDefaults, pullModel, pullModelById, chooseAvatarImage, clearAvatarImage, saveAvatarSettings, createSkillFromSettings, uploadSkillMd, activateSkill, toggleSkill, setControl, setBrowserAgent, addMcp, listMcpTools, toggleMcp, chooseObsidianVault, createObsidianTestNote, saveAppearance, chooseBackgroundImage, clearBackgroundImage, setBackgroundPreset, renderDiagnostics, runDiagnostics, openScreenPicker, loadScreenSources, chooseScreenSource, stopScreenLive, finishOnboarding});
...
try{
  ...
  $('#engineBadge')?.addEventListener('click',()=>openSettingsTab('universal-ai'));
