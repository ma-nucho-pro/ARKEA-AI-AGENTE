...
## Modo standalone de desarrollo

En Windows ejecuta `scripts\start_windows.bat`. Este modo desactiva
explícitamente el token **solo para desarrollo en loopback** y no debe
exponerse a la red. La aplicación Electron y el backend empaquetado conservan
autenticación obligatoria.

Para una ejecución manual equivalente:
...
Después abre `http://127.0.0.1:7210`. Sin `ARKEA_DEV_NO_AUTH=1`, el backend
genera un token aleatorio y un navegador normal no puede abrir la interfaz.
