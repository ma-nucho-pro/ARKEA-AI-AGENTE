...
from backend.arkea_core.security import guarded_request, read_upload_limited

router = APIRouter(prefix="/api/arkea/voice", tags=["voice"])

class TTSIn(BaseModel):
    text: str = Field(min_length=1, max_length=10_000)
    voice_id: str | None = Field(default=None, pattern=r"^[A-Za-z0-9_-]{1,100}$")
...
def elevenlabs_tts(body: TTSIn):
    ...
    if not re.fullmatch(r"[A-Za-z0-9_-]{1,100}", voice_id):
        raise HTTPException(400, "Voice ID no válido")
    ...
    r = guarded_request("POST", url, max_response_bytes=25 * 1024 * 1024, headers={"xi-api-key": api_key, "Content-Type":"application/json"}, json=payload, timeout=90)
    if not r.ok:
...
def elevenlabs_voices():
    ...
    r = guarded_request("GET", "https://api.elevenlabs.io/v1/voices", headers={"xi-api-key": api_key}, timeout=45)
    if not r.ok:
...
async def transcribe_audio(file: UploadFile = File(...), language: str = Form('es')):
    ...
    if api and api.get('api_key') and api.get('base_url'):
        try:
            ...
            allow_local = (api.get('provider') or '').lower() == 'local_whisper'
            r = guarded_request("POST", api['base_url'], allow_local=allow_local, headers=headers, files=files, data=data, timeout=180)
            r.raise_for_status()
            jd = r.json()
