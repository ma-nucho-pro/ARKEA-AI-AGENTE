...
class ApiConnectionIn(BaseModel):
    api_type: str = Field(min_length=1, max_length=50)
    provider: str = Field(min_length=1, max_length=100)
    display_name: str = Field(default="default", max_length=160)
...
def test_api(api_id: int):
    ...
    allow_local = provider in ("omniroute", "ollama_local", "ollama_vision", "comfyui_local", "automatic1111_local", "local_whisper")
    ...
    try:
        if provider in ("ollama_local", "ollama_vision") or "127.0.0.1:11434" in base_url or "localhost:11434" in base_url:
            ...
            r = guarded_request("POST", base_url, allow_local=True, json=payload, timeout=45)
        ...
        if api_type in ("chat", "vision", "translation", "code", "artifact", "document", "file", "web_search"):
            ...
            r = guarded_request("POST", base_url, allow_local=allow_local, headers=_headers(api_key, extra), json=payload, timeout=45)
        ...
        if api_type in ("image", "image_generation"):
            ...
            r = guarded_request("POST", base_url, allow_local=allow_local, headers=_headers(api_key, extra), json=payload, timeout=90)
        ...
        if api_type == "voice_tts":
            if "elevenlabs" in provider or "{voice_id}" in base_url:
                ...
                r = guarded_request("POST", url, headers={"xi-api-key": api_key, "Content-Type":"application/json"}, json=payload, timeout=45)
        ...
        if api_type in ("video", "automation", "embedding", "rerank", "custom"):
            ...
            try:
                r = guarded_request("OPTIONS", base_url, allow_local=allow_local, headers=_headers(api_key, extra), timeout=20)
            except Exception:
                r = guarded_request("GET", base_url, allow_local=allow_local, headers=_headers(api_key, extra), timeout=20)
...
def save_api(body: ApiConnectionIn):
    ...
    if existing:
        if body.api_key:
            ...
            execute("UPDATE api_connections SET base_url=?, model_id=?, extra_json=?, enabled=?, updated_at=CURRENT_TIMESTAMP WHERE id=?", (body.base_url, body.model_id, extra, int(body.enabled), existing['id']))
...
def cleanup_broken():
    ...
    for api in rows("SELECT id,provider,base_url,api_key FROM api_connections"):
        is_openrouter = "openrouter" in (api.get("provider") or "").lower() or "openrouter.ai" in (api.get("base_url") or "").lower()
        if is_openrouter and len((api.get("api_key") or "").strip()) < 20:
            execute("UPDATE api_connections SET enabled=0, updated_at=CURRENT_TIMESTAMP WHERE id=?", (api["id"],))
...
def templates():
    ...
    return {
        ...
        "providers": {
            "chat":[
                {"provider":"omniroute","base_url":"http://127.0.0.1:20128/v1/chat/completions","model":"auto"},
            ...
            "code":[
                {"provider":"omniroute","base_url":"http://127.0.0.1:20128/v1/chat/completions","model":"auto/coding"},
            ...
            "image_generation":[
                {"provider":"omniroute","base_url":"http://127.0.0.1:20128/v1/images/generations","model":"auto"},
